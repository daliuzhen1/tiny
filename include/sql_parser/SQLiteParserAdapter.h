...
class SQLiteParserDB
{
public:
   static std::shared_ptr<SQLiteParserDB> getInstance()
   {
       if (_instance)
           return _instance;
       _instance = std::shared_ptr<SQLiteParserDB>(new SQLiteParserDB);
       return _instance;
   }
   sqlite3* getDB()
   {
...
private:
   ...
   static std::shared_ptr<SQLiteParserDB> _instance;
   sqlite3 *_db;
   SQLiteParserDB(): _db(nullptr)
   {
   }
};
std::shared_ptr<SQLiteParserDB> SQLiteParserDB::_instance;

template <char const* moduleName, int version>
...
public:
    ...
    bool startUp()
    {
        sqlite3* dbPtr = SQLiteParserDB::getInstance()->getDB();
        if (_startUped)
            return true;
        if (!dbPtr)
            return false;
        _dbPtr = dbPtr;
        _startUped = true;
        int rc = sqlite3_create_module(_dbPtr, moduleName, &_sqlite3_module, nullptr);
        return rc == SQLITE_OK;
    }

    bool createVirtualTable(const std::string &tableName)
    {
        std::string statement = "CREATE VIRTUAL TABLE ";
        statement += tableName;
        statement += " using ";
        statement += moduleName;
        sqlite3 *db = SQLiteParserDB::getInstance()->getDB();
        int rc = sqlite3_exec(db, statement.c_str(), nullptr, nullptr, nullptr);
        return rc == SQLITE_OK;
    }

    static std::shared_ptr<SQLiteParserAdapter<moduleName, version>> getInstance()
    {
        if (_instance)
            return _instance;
        _instance = std::shared_ptr<SQLiteParserAdapter<moduleName, version>>(new SQLiteParserAdapter<moduleName, version>());
        return _instance;
    }

    typedef std::function<int(sqlite3*, void *pAux,
        int argc, const char *const*argv,
        sqlite3_vtab **ppVTab, char**, std::vector<ColumnInfo>& colInfos)> CreateCallBack;
    typedef std::function<int(sqlite3*, void *pAux,
        int argc, const char *const*argv,
...
private:
    ...
    SQLiteParserAdapter::SQLiteParserAdapter() : _startUped(false), _version(version)
    {
        _sqlite3_module = {};
        _sqlite3_module.iVersion = _version;
    }
        ...
        sqlite3_vtab **ppVTab, char** pzErr)
    {
        if (_instance->_createCallBack)
        {
            ...
            int rc = _instance->_createCallBack(db, pAux,
                argc, argv,
                ppVTab, pzErr, colInfos);
            if (rc != SQLITE_OK)
                return rc;
            ...
            schemaStr += ")";
            rc = sqlite3_declare_vtab(db, schemaStr.c_str());
            return rc;
        }
        return -1;
    }
...
template <char const* moduleName, int version>
std::shared_ptr<SQLiteParserAdapter<moduleName, version>> SQLiteParserAdapter<moduleName, version>::_instance;
