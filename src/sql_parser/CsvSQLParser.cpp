...
struct SourceParser
{
    ...
    bool createOrReloadSource()
    {
        ...
        _pExtractor->selectTable(_pExtractor->getTableList()->at(0));
        _currIndex = 0;
        _lastPack = false;
...
void CsvSQLParser::initilize()
{
    ...
    {
        sqlite3_cursor_csv* cur = static_cast<sqlite3_cursor_csv*>(pCur);
        if(idx >= 0 && idx < cur->p_csvWrapper->_curr_pack_data.size())
        {
          sqlite3_result_text(ctx, cur->p_csvWrapper->_curr_pack_data.at(idx)->at(cur->p_csvWrapper->_currIndex).c_str(), -1, SQLITE_STATIC);
        }
        return SQLITE_OK;
    });
